...
exports.WishlistLambda = async (event) => {
            ...
            if(task === 'ADD'){
                await DBConnection.send(
                    new UpdateCommand({
                        ...
                        UpdateExpression: "SET wishlist = list_append(wishlist,:v)",
                ...
                const updatedwishlist = userdetails.Item.wishlist.filter(item => item != productid)
                await DBConnection.send(
                    new UpdateCommand({
                        TableName: process.env.UserDetailsTable,
                        Key:{user_id:result.Item.user_id},
                        UpdateExpression:"SET wishlist = :v",
                        ExpressionAttributeValues: {
                            ":v":updatedwishlist
                        },
                        ReturnValues: "UPDATED_NEW"
    ...
    if( event.httpMethod === 'GET'){
        if(result.Item){
            ...
            if(userdetails.Item){
                return corsheaders({
                    statusCode: 200,
                    body: JSON.stringify({result:userdetails.Item.wishlist})
                })
            }
